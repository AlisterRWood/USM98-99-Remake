...
struct FileScreen:View {
    ...
    var body:some View {ClassicScreen(title:"File & Options") {
        HStack {Button("Save current career") {store.save();refresh()};Button("New game") {store.newCareer=true};Button("Main menu") {store.title()};Button("Import save") {store.importSave()};Button("Export save") {store.exportSave()}}
...
struct HelpScreen:View {
    ...
    let text:[String:String]=[
        "Getting started":"Choose New game from the title screen. Enter your name, choose a country, division and club, then your starting funds and role. Manager controls the whole club; Coach delegates finances to the board.",
        ...
        "Saving":"The game saves automatically after changes and matches. File offers named save slots, load, import and export. New careers back up your previous current save. You can save a match in progress and resume it paused."]
