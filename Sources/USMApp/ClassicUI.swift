        ...
        ClassicScreen(title:"Individual Training") {
            ...
            ScrollView {LazyVStack(spacing:6) {ForEach(store.career.orderedSquad) {p in
                HStack {Text(p.name).frame(width:160,alignment:.leading)
                    GameChoice(label:"",value:Binding(get:{let id=store.career.managementState.assignments[p.id]?.coachID;return store.career.managementState.staff.first{$0.id==id}?.assignmentLabel ?? "Unassigned"},set:{name in store.career.initializeManagement();let coach=store.career.management!.staff.first{$0.assignmentLabel==name && $0.employed};if let coach {var a=store.career.management!.assignments[p.id] ?? TrainingAssignment(coachID:coach.id,skill:2);a.coachID=coach.id;store.career.management!.assignments[p.id]=a}else{store.career.management!.assignments.removeValue(forKey:p.id)};store.save()}),options:["Unassigned"]+store.career.managementState.staff.filter{$0.employed && $0.speciality != "Scout"}.map(\.assignmentLabel)).frame(minWidth:330)
                    GameChoice(label:"",value:Binding(get:{skills[store.career.managementState.assignments[p.id]?.skill ?? 2]},set:{value in store.career.initializeManagement();if var a=store.career.management!.assignments[p.id]{a.skill=skills.firstIndex(of:value) ?? 2;store.career.management!.assignments[p.id]=a;store.save()}}),options:skills)
                    GameNumber(label:"Intensity",value:Binding(get:{store.career.managementState.assignments[p.id]?.intensity ?? 1},set:{value in store.career.initializeManagement();if var a=store.career.management!.assignments[p.id]{a.intensity=value;store.career.management!.assignments[p.id]=a;store.save()}}),range:1...3).frame(width:240)
    ...
    var body:some View {
        ClassicScreen(title:advertising ? "Advertising Boards & Programme":"Club Sponsors") {
            ...
            ScrollView {LazyVStack(spacing:7) {ForEach(store.career.managementState.deals.filter{advertising ? $0.kind != "Club sponsor":$0.kind=="Club sponsor"}) {d in
                HStack {VStack(alignment:.leading){Text(d.brand).font(.title3.bold());Text(d.kind).foregroundStyle(muted)};Spacer();Text(money(d.weekly)+" / week");Text("\(d.accepted ? d.remaining:d.term) weeks").frame(width:110);Button(d.accepted ? "Contract active":"Accept offer") {if !store.career.acceptDeal(d.id){store.message="An existing sponsor contract must expire first."};store.save()}.disabled(d.accepted)}.padding(15).background(.black.opacity(0.25),in:RoundedRectangle(cornerRadius:6))
            }}}
            Text(advertising ? "Pitch-board contracts and programme advertising are separate revenue sources.":"Current partner: \(store.career.sponsor)").foregroundStyle(mint)
        }
    }
}
